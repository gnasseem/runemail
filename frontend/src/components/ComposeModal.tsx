...
export default function ComposeModal({
  ...
  const [to, setTo] = useState(() => {
    ...
    if (replyTo?._replyAll) {
      ...
      for (const addr of extractReplyAddresses(
        replyTo.recipients || "",
        userEmail,
      ))
        addrs.add(addr);
  ...
  const [cc, setCc] = useState(() => {
    if (draft?.cc_addresses?.length)
      return (draft.cc_addresses as string[]).join(", ");
  ...
  const [bcc, setBcc] = useState(() =>
    draft?.bcc_addresses?.length
      ? (draft.bcc_addresses as string[]).join(", ")
      : "",
  );
  const [showCcBcc, setShowCcBcc] = useState(
    !!(
      ...
      draft?.bcc_addresses?.length ||
      (replyTo?._replyAll && replyTo?.cc_recipients)
    ),
  );
  ...
  const [rightPanel, setRightPanel] = useState<null | "meeting" | "templates">(
    null,
  );
  ...
  // Convert ISO scheduleAt to datetime-local format for the picker, and back
  ...
  function datetimeLocalToIso(dtl: string): string {
    if (!dtl) return "";
    const [datePart, timePart] = dtl.split("T");
  ...
  // Context-aware knowledge entries for current recipient
  const [recipientKnowledge, setRecipientKnowledge] = useState<
    { entity: string; entity_type: string; info: string }[]
  >([]);
  useEffect(() => {
    const primaryTo = to.split(",")[0]?.trim();
    if (!primaryTo || !primaryTo.includes("@")) {
      setRecipientKnowledge([]);
      return;
    }
    const domain = primaryTo.split("@")[1] ?? "";
    const firstName = primaryTo.split("@")[0].replace(/[^a-z]/gi, "") ?? "";
    const filters: string[] = [];
    if (firstName.length > 2)
      filters.push(`entity.ilike.%${firstName}%`, `info.ilike.%${firstName}%`);
    if (domain.length > 3)
      filters.push(`entity.ilike.%${domain}%`, `info.ilike.%${domain}%`);
    if (filters.length === 0) {
      setRecipientKnowledge([]);
      return;
    }
    supabase
      ...
      .or(filters.join(","))
      .limit(5)
      .then(({ data }) => setRecipientKnowledge(data ?? []));
  }, [to, user.id]);
  ...
  // meetingDatetime = "YYYY-MM-DDTHH:MM"
  const [meetingDatetime, setMeetingDatetime] = useState(
    `${nowDate}T${nowTime}`,
  );
  const meetingDate = meetingDatetime.split("T")[0] || nowDate;
  const meetingTime = meetingDatetime.split("T")[1] || nowTime;
  ...
  >([]);
  const [appliedSignatureId, setAppliedSignatureId] = useState<string | null>(
    null,
  );
  const SIGNATURE_SEPARATOR =
    '<div data-runemail-sig="1" style="margin-top:16px;padding-top:12px;border-top:1px solid #e5e7eb;">';

  const getDefaultSignatureForAccount = (
    accountId: string,
    sigs: typeof allSignatures,
  ) => {
    // First try exact account match, then fall back to global (null account), then first default
    return (
      sigs.find((s) => s.is_default && s.gmail_account_id === accountId) ??
      sigs.find((s) => s.is_default && !s.gmail_account_id) ??
      null
    );
  };

  const stripSignature = (html: string) => {
    const idx = html.indexOf(SIGNATURE_SEPARATOR);
    return idx >= 0 ? html.slice(0, idx) : html;
  };

  const appendSignature = (html: string, sig: { html: string } | null) => {
    const base = stripSignature(html);
    if (!sig) return base;
    return base + SIGNATURE_SEPARATOR + sig.html + "</div>";
  };
  ...
  useEffect(() => {
    ...
    // Load signatures (only for new emails, not replies/drafts)
    if (!replyTo && !draft) {
      (async () => {
        const {
          data: { session },
  ...
  // Apply default signature when account or signatures change (new compose only)
  useEffect(() => {
    if (replyTo || draft || allSignatures.length === 0) return;
    const effectiveAccountId = selectedAccountId || gmailAccounts[0]?.id || "";
    const sig = getDefaultSignatureForAccount(
      effectiveAccountId,
      allSignatures,
    );
    if (sig && sig.id !== appliedSignatureId) {
      setBody((prev) => appendSignature(prev, sig));
      setAppliedSignatureId(sig.id);
    } else if (!sig && appliedSignatureId) {
      setBody((prev) => stripSignature(prev));
      setAppliedSignatureId(null);
    }
  }, [selectedAccountId, allSignatures, gmailAccounts]);
      ...
      try {
        ...
        if (!ready) {
          if (aiMode === "local") {
            addToast(
              "error",
              "WebLLM not loaded. Make sure local AI is initialized.",
            );
            ...
            draftContext: {
              ...
              replyTo: replyTo
                ? {
                    ...
                    body: replyTo.body_text?.slice(0, 800) || "",
                  }
                : undefined,
            },
          });
      ...
      const res = await fetch(`${apiUrl}/draft-email`, {
        ...
        body: JSON.stringify({
          ...
          replyTo: replyTo
            ? {
                subject: replyTo.subject,
                body: replyTo.body_text?.slice(0, 1000),
              }
            : undefined,
        }),
      });
  ...
  const refineWithFeedback = async () => {
    if (!refineFeedback.trim() || !body) return;
    setRefining(true);
    try {
      const apiUrl = process.env.NEXT_PUBLIC_SUPABASE_URL + "/functions/v1/api";
      const {
      ...
      const res = await fetch(`${apiUrl}/refine-draft`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session?.access_token}`,
      ...
      if (res.ok) {
        ...
        if (data.draft) {
          ...
          addToast(
            "success",
            "Draft updated. Feedback saved to your knowledge base.",
          );
        } else {
          addToast("error", "Could not refine draft");
        }
      } else {
        addToast("error", "Could not refine draft");
      }
  ...
  const createMeetingRecord = async () => {
    ...
    if (isNaN(durationMin) || durationMin <= 0) {
      addToast("error", "Enter a valid meeting duration");
      return;
    }
    ...
    if (createZoomLink) {
      try {
        ...
        console.error("[ComposeModal] Zoom creation:", err);
        addToast("info", "Could not create Zoom link");
      }
    }
    ...
    try {
      ...
      console.error("[ComposeModal] Calendar creation:", err);
    }
...
<div style="font-family:Google Sans,Roboto,Arial,sans-serif;max-width:600px;margin:20px 0">
  ...
  const handleSend = async (bodyOverride?: string) => {
    ...
    try {
      // Resolve the "From" account once and use it for every branch (schedule,
      // undo-send, immediate). Previously schedule and undo-send fell back to
      // accounts[0], ignoring the selection.
      const parsedTo = to
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean);
      ...
      const parsedBcc = bcc
        ? bcc
            ...
            .filter(Boolean)
        : [];
      const emailOk = (addr: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(addr);
      const allRecipientsValid = [...parsedTo, ...parsedCc, ...parsedBcc].every(
        emailOk,
      );
      if (parsedTo.length === 0 || !allRecipientsValid) {
        addToast("error", "Please enter valid recipient email addresses");
        setSending(false);
        return;
      }
      ...
      if (scheduleAt) {
        if (draft?._scheduledId) {
          const { error: upErr } = await supabase
            ...
            .update({
              gmail_account_id: effectiveAccountId,
              to_addresses: parsedTo,
              cc_addresses: parsedCc.length ? parsedCc : null,
              bcc_addresses: parsedBcc.length ? parsedBcc : null,
              subject,
              body_html: sendBody,
              send_at: scheduleAt,
              status: "pending",
          ...
          if (upErr) {
            addToast("error", "Failed to update scheduled email");
            setSending(false);
            return;
          }
        } else {
          ...
          if (insErr) {
            addToast("error", "Failed to schedule email");
            setSending(false);
            return;
          }
        }
      } else if (attachments.length === 0 && !trackOpens) {
        // Undo Send: queue in scheduled_emails with 30-second window (no attachments/tracking)
        const { data: scheduled, error: undoErr } = await supabase
          .from("scheduled_emails")
          .insert({
            user_id: user.id,
            gmail_account_id: effectiveAccountId,
        ...
        if (undoErr || !scheduled?.id) {
          addToast("error", "Failed to queue send");
          setSending(false);
          return;
        }

        const scheduledId = scheduled.id;
        addToast("info", "Sending in 30s...", {
          duration: 30_000,
          action: {
            label: "Undo",
            onClick: async () => {
              if (scheduledId) {
                await supabase
                  ...
                  .eq("id", scheduledId)
                  .eq("status", "pending");
                addToast("success", "Send cancelled");
              }
            },
          },
        });
      } else {
        // Immediate send: attachments or read tracking present
        const attachmentData = await Promise.all(
          attachments.map(
            (file) =>
              new Promise<{ name: string; contentType: string; data: string }>(
                (resolve) => {
        ...
        try {
          sendRes = await fetch(`${apiUrl}/send-email`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${session?.access_token}`,
          ...
          console.error("[compose/send] network error:", netErr);
          addToast("error", "Failed to send: network error");
          setSending(false);
          return;
        }
        if (!sendRes.ok) {
      ...
      if (allRecipients.length > 0) {
        const newContacts = allRecipients.map((email) => ({
      ...
      if (trackOpens)
        notifyReceipt({ subject, recipient_email: to.split(",")[0].trim() });
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          maxWidth: `${680 + (rightPanel === "templates" ? 256 : rightPanel === "meeting" ? 240 : 0)}px`,
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-auto flex flex-col min-w-0">
            <div className="flex-1 overflow-auto px-4 pt-3 pb-2 space-y-2">
              ...
              {/* AI refine bar — shown after AI generation */}
              {aiGenerated && (
                <div className="mx-2 rounded-lg border border-[var(--border)] bg-slate-50 dark:bg-slate-800/60 overflow-hidden">
                  {!showRefinePanel ? (
                    <button
                      onClick={() => setShowRefinePanel(true)}
                      className="w-full flex items-center gap-2 px-3 py-2 text-xs text-[var(--muted)] hover:text-[var(--accent)] hover:bg-slate-100 dark:hover:bg-slate-700/60 transition-colors"
                    >
                      <span
                        className="material-symbols-outlined"
                        style={{ fontSize: "14px" }}
                      ...
                      Not right? Tell the AI what to change
                      <span
                    ...
                    <div className="p-2.5 space-y-2">
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <button
                          ...
                          className="ml-auto text-[var(--muted)] hover:text-slate-900 dark:hover:text-white"
                        >
                          <span
                            className="material-symbols-outlined"
                            style={{ fontSize: "14px" }}
                      ...
                      <textarea
                        ...
                        placeholder="e.g. Make it shorter, use a more casual tone, add a question at the end..."
                        rows={2}
                        className="w-full px-2 py-1.5 rounded border border-[var(--border)] bg-[var(--background)] text-xs focus:outline-none focus:border-[var(--accent)] resize-none"
                        autoFocus
                      />
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-[var(--muted)]">
                          Cmd+Enter to apply. Feedback is saved to your
                          knowledge base.
                        </span>
                        <button
                          onClick={refineWithFeedback}
                          disabled={refining || !refineFeedback.trim()}
                          className="ml-auto flex items-center gap-1 px-2.5 py-1 rounded-full bg-[var(--accent)] text-white text-xs font-medium hover:opacity-90 disabled:opacity-50"
                        >
                          <span
                            className={`material-symbols-outlined ${refining ? "animate-spin" : ""}`}
                            style={{ fontSize: "13px" }}
                          >
                            {refining ? "progress_activity" : "auto_awesome"}
                          </span>
              ...
              {attachments.length > 0 && (
                <div className="flex flex-wrap gap-1.5 px-2 pb-2">
                  {attachments.map((file, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-1 px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-xs text-[var(--foreground)]"
                    >
                      <span
                        className="material-symbols-outlined"
                        style={{ fontSize: "13px" }}
                      ...
                      <button
                        ...
                        className="ml-0.5 text-[var(--muted)] hover:text-red-500"
                      >
                        <span
                          className="material-symbols-outlined"
                          style={{ fontSize: "13px" }}
            ...
            <div className="flex items-center gap-1.5 px-3 py-2 border-t border-[var(--border)] shrink-0">
              ...
              >
                <span
                  className={`material-symbols-outlined ${generatingDraft ? "animate-spin" : ""}`}
                  style={{ fontSize: "16px" }}
                >
              ...
              <div className="flex items-center gap-0.5 ml-1">
                ...
                <button
                  ...
                  title={
                    !to || !subject
                      ? "Enter recipient and subject first"
                      : "Schedule meeting"
                  }
                >
                  <span
                ...
                <div className="relative">
                  ...
                  {showSchedulePopup && (
                    <div
                      className="absolute bottom-full mb-2 right-0 z-50 bg-[var(--background)] border border-[var(--border)] rounded-xl shadow-2xl p-3 w-80"
                    ...
                    >
                      <div className="flex items-center justify-between mb-2.5">
                        ...
                        <button
                          ...
                          className="text-[var(--muted)] hover:text-slate-900 dark:hover:text-white"
                        >
                          <span
                            className="material-symbols-outlined"
                            style={{ fontSize: "15px" }}
                      ...
                      <DateTimePicker
                        value={isoToDatetimeLocal(scheduleAt)}
                        onChange={(dtl) =>
                          setScheduleAt(datetimeLocalToIso(dtl))
                        }
          ...
          {rightPanel === "meeting" && (
            <div className="w-60 border-l border-[var(--border)] overflow-auto p-3 space-y-2.5 bg-slate-50 dark:bg-slate-800/50 shrink-0">
              ...
              <div className="text-xs text-[var(--muted)] bg-[var(--background)] rounded px-2 py-1.5 border border-[var(--border)] space-y-0.5">
                ...
                <div className="truncate">
                  <span className="font-medium">Title:</span> {subject || "—"}
                </div>
              </div>
              <DateTimePicker
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  ...
                  <button
                    key={value}
                    type="button"
                    onClick={() => setMeetingDuration(value)}
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-colors ${
                      meetingDuration === value
                        ? "bg-[var(--accent)] text-white border-[var(--accent)]"
                        : "border-[var(--border)] text-slate-600 dark:text-slate-400 hover:border-[var(--accent)] hover:text-[var(--accent)]"
                    }`}
                  >
                    {label}
                  </button>
              ...
              <div className="flex-1 overflow-auto">
                {templates.length === 0 ? (
                  <p className="text-xs text-[var(--muted)] px-3 py-3">
                    No templates yet. Save your current message as one below.
                  </p>
                ) : (
                  templates.map((t) => (
                    <div
                      key={t.id}
                      className="flex items-center justify-between px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 border-b border-[var(--border)] last:border-0"
                    >
                      <button
                        onClick={() => applyTemplate(t)}
                        className="text-[12px] text-slate-900 dark:text-white text-left flex-1 truncate"
                      >
                        {t.name}
                      </button>
                      <button
                        onClick={() => deleteTemplate(t.id)}
                        className="text-[var(--muted)] hover:text-red-500 ml-2 shrink-0"
                      >
                        <span
                          className="material-symbols-outlined"
                          style={{ fontSize: "14px" }}
