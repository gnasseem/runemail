...
const aiItems = [
  ...
  { id: "followups", icon: "schedule_send", label: "Follow-ups" },
...
function NavGroup({ label, items, view, setView, setSidebarOpen, badges }: {
  label: string;
...
export default function Sidebar() {
  const { view, setView, sidebarOpen, setSidebarOpen, openCompose, briefingVersion } = useApp();
  ...
  useEffect(() => {
    if (briefingVersion > prevBriefingVersion.current) {
      ...
      if (view !== "briefing") setBriefingBadge(true);
    }
  }, [briefingVersion, view]);

  // Clear badge when user navigates to briefing
  useEffect(() => {
    if (view === "briefing") setBriefingBadge(false);
  }, [view]);
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 px-2 overflow-y-auto">
          ...
          <NavGroup label="AI Features" items={aiItems} view={view} setView={setView} setSidebarOpen={setSidebarOpen} badges={{ briefing: briefingBadge }} />
