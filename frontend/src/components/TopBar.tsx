...
const STATUS_CHIPS = [
  {
    ...
    activeClass:
      "border-slate-400 bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-600",
  },
  {
    key: "starred",
    label: "Starred",
...
const CATEGORY_CHIPS = [
  ...
  },
  {
    ...
    dot: "#f59e0b",
    activeClass:
      "border-amber-300 bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800",
  },
  {
    key: "newsletter",
    label: "Newsletter",
    dot: "#a855f7",
    activeClass:
      "border-violet-300 bg-violet-50 text-violet-600 dark:bg-violet-950/40 dark:text-violet-400 dark:border-violet-800",
  },
  {
    key: "informational",
    label: "Informational",
    dot: "#3b82f6",
    activeClass:
      "border-sky-300 bg-sky-50 text-sky-600 dark:bg-sky-950/40 dark:text-sky-400 dark:border-sky-800",
  },
];
...
export default function TopBar() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (
        feedbackRef.current &&
        !feedbackRef.current.contains(e.target as Node)
      )
  ...
  const sendFeedback = async () => {
    ...
    try {
      const feedbackRecipient =
        process.env.NEXT_PUBLIC_FEEDBACK_EMAIL || "feedback@runemail.app";
      const {
        data: { session },
      } = await supabase.auth.getSession();
      const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
      // Escape user-supplied content so HTML metacharacters don't end up in
      // the outgoing email body as live markup.
      const escapeHtml = (s: string) =>
        s
          ...
          .replace(/"/g, "&quot;")
          .replace(/'/g, "&#39;");
      const safeFrom = escapeHtml(user?.email ?? "unknown");
      const safeBody = escapeHtml(feedbackText).replace(/\n/g, "<br>");
      const res = await fetch(`${supabaseUrl}/functions/v1/api/send-email`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          subject: `RuneMail Feedback from ${user?.email ?? "user"}`,
          body_html: `<p><strong>From:</strong> ${safeFrom}</p><p><strong>Feedback:</strong></p><p>${safeBody}</p>`,
        }),
      });
      if (!res.ok) {
        addToast?.("error", "Failed to send feedback");
        return;
      }
      setFeedbackText("");
      setShowFeedback(false);
      addToast?.("success", "Thanks for the feedback!");
    } catch (err) {
      console.error("[topbar/feedback] network error:", err);
      addToast?.("error", "Failed to send feedback");
    } finally {
  ...
  return (
    <header
      className="h-13 px-4 flex items-center justify-between border-b bg-[var(--background)] border-[var(--border)] shrink-0 z-30"
      style={{ height: "52px" }}
    >
      {/* Left: hamburger + logo */}
            ...
            <div className="relative shrink-0" ref={filterMenuRef}>
              ...
              {showFilterMenu && (
                <div className="absolute right-0 top-full mt-1 bg-[var(--surface)] border border-[var(--border)] rounded-lg shadow-lg p-1.5 z-50 min-w-[185px] flex flex-col gap-0.5">
                  {/* Status section */}
                  <p className="px-2 pt-0.5 pb-1 text-[10px] font-semibold uppercase tracking-wider text-[var(--muted)]">
                    Status
                  </p>
                  ...
                  {/* Categories section */}
                  <p className="px-2 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-wider text-[var(--muted)] border-t border-[var(--border)] mt-0.5">
                    Categories
                  </p>
                  {[
                    ...CATEGORY_CHIPS,
                    ...inboxCustomCategories.map((c) => ({
                      key: c,
                      label: c,
                      dot: "#6b7280",
                      activeClass:
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        ...
        <div className="relative" ref={feedbackRef}>
          <button
            ...
            className="flex items-center justify-center p-2 rounded-lg hover:bg-[var(--surface-2)] text-[var(--muted)] transition-colors focus:outline-none font-bold text-[13px] leading-none"
            title="Report an issue"
          >
            !!
          </button>
          {showFeedback && (
            <div className="absolute right-0 top-full mt-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-lg p-3 z-50 w-72 flex flex-col gap-2">
              ...
              <textarea
                ...
                rows={4}
                className="w-full resize-none rounded-lg border border-[var(--border)] bg-[var(--surface-2)] text-[12px] text-[var(--foreground)] placeholder:text-[var(--muted)] px-2.5 py-2 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/25 focus:border-[var(--accent)]/40"
              />
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-[12px] font-medium bg-[var(--accent)] text-white hover:opacity-90 disabled:opacity-40 transition-opacity"
                >
        ...
        {/* Auto-Resolve agent status pill (only when a session is active) */}
        {agentSession &&
          ["planning", "asking", "executing", "ready"].includes(
            agentSession.status,
          ) && (
            <button
              onClick={() => {
              ...
              className={`group cursor-pointer relative flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-medium text-white transition-colors ${
              ...
              title={
                agentSession.status === "asking"
                  ? "Auto-Resolve needs your input — click to answer"
                  : "Auto-Resolve is running"
              }
            >
              <span
                className="material-symbols-outlined"
              ...
              {(agentSession.status === "planning" ||
                agentSession.status === "executing") && (
                <span className="solve-dot-wave text-white inline-flex">
                  <span />
                  <span />
                  <span />
                </span>
