...
import { SCRIPTED_PROMPTS, getScriptedResponse, type ScriptedPrompt } from "./tutorialScripts";
...
export default function TutorialAssistantPanel({ open, onClose, onAction }: Props) {
  ...
  const streamResponse = useCallback((scripted: ScriptedPrompt) => {
    const fullText = scripted.response;
    let index = 0;
    setMessages((prev) => [
      ...prev,
      { role: "assistant", content: "", streaming: true, actions: scripted.actions },
    ]);
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)] shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-[var(--accent)] flex items-center justify-center">
            <span className="material-symbols-outlined text-white dark:text-[#202124]" style={{ fontSize: "15px", fontVariationSettings: "'FILL' 1" }}>
              assistant
            </span>
          </div>
          <span className="text-[14px] font-bold text-[var(--foreground)]">AI Assistant</span>
          ...
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            {msg.role === "assistant" && (
              <div className="w-6 h-6 rounded-full bg-[var(--accent)] flex items-center justify-center shrink-0 mt-0.5 mr-2">
                <span className="material-symbols-outlined text-white dark:text-[#202124]" style={{ fontSize: "12px", fontVariationSettings: "'FILL' 1" }}>
                  assistant
                </span>
              </div>
            )}
                  ...
                  {msg.actions.map((action, j) => (
                    ...
                    >
                      <span className="material-symbols-outlined" style={{ fontSize: "11px", fontVariationSettings: "'FILL' 1" }}>
                        {action.type === "meeting" ? "event_available" : action.type === "todo" ? "check_circle" : "edit"}
                      </span>
                      {action.label}
                    </span>
                  ))}
        ...
        {loading && messages[messages.length - 1]?.role !== "assistant" && (
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-[var(--accent)] flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-white dark:text-[#202124]" style={{ fontSize: "12px", fontVariationSettings: "'FILL' 1" }}>assistant</span>
            </div>
      ...
      {showSuggestions && messages.length <= 1 && (
        <div className="px-4 pb-2 flex flex-col gap-2 shrink-0">
          ...
          {SCRIPTED_PROMPTS.map((prompt, i) => (
            ...
            >
              <span className="material-symbols-outlined text-[var(--accent)] mr-1 align-middle group-hover:opacity-100 opacity-70" style={{ fontSize: "12px" }}>
                auto_awesome
              </span>
              {prompt.label}
            </button>
