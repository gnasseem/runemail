...
export default function TutorialChapterCard({ chapter, title, subtitle, bullets = [], visible, onContinue }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Gradient bg card */}
        <div className={`w-full rounded-3xl p-8 bg-gradient-to-br ${config.accent} border border-white/10 max-h-[90vh] overflow-y-auto`}
            ...
            <ul className="mb-6 flex flex-col gap-2.5">
              {bullets.map((b, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span
                    className="material-symbols-outlined text-[var(--accent)] mt-0.5 shrink-0"
                    style={{ fontSize: "16px", fontVariationSettings: "'FILL' 1" }}
                  >
                    {b.icon}
                  </span>
                  <span className="text-[13px] text-[var(--muted)] leading-snug">{b.text}</span>
                </li>
              ))}
            </ul>
          )}
