...
export default function TutorialComposeModal({ onClose, onSend }: Props) {
  const [to, setTo] = useState(MOCK_DRAFT.to);
  const [subject, setSubject] = useState(MOCK_DRAFT.subject);
  ...
  const handleAiDraft = () => {
    ...
    // Simulate streaming AI completion
    let i = 0;
  ...
  return (
    <div className="fixed inset-0 z-[415] flex items-end justify-end" style={{ pointerEvents: "none" }}>
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/20" style={{ pointerEvents: "all" }} onClick={onClose} />

      {/* Modal */}
      <div
        className="relative w-full max-w-lg mb-0 mr-0 rounded-t-2xl border border-[var(--border)] shadow-2xl flex flex-col overflow-hidden"
        style={{
          height: "560px",
