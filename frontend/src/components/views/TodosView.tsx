          ...
          {loadingSuggestions ? (
            ...
            <div className="divide-y divide-[var(--border)]">
              {pendingSuggestions.map((s, i) => (
                <div key={`${s.emailId}-${i}`} className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 group">
                  {/* Header row: category badge + time */}
                  <div className="flex items-center justify-between mb-1.5">
                    <span className={`inline-block text-[9px] px-1.5 py-0.5 rounded font-medium ${CATEGORY_COLORS[s.category] || "bg-slate-100 text-slate-500"}`}>
                      {s.category}
                    </span>
                  ...
                  {/* Email context card */}
                  <div
                    className="rounded-md border border-[var(--border)] bg-slate-50 dark:bg-slate-800/40 px-2.5 py-2 mb-2.5 cursor-pointer hover:border-[var(--accent)]/50 transition-colors"
                    onClick={() => openEmailById(s.emailId)}
                    title="Open source email"
                  >
                    <p className="text-xs font-medium text-slate-700 dark:text-slate-300 leading-snug mb-0.5">
                      {s.emailSubject}
                    </p>
                    {s.emailSender && (
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--accent)] text-white hover:opacity-90"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-0.5 px-2 py-1.5 rounded-lg text-xs text-[var(--muted)] hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-opacity"
                      title="Dismiss"
                    >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-[var(--border)] flex-shrink-0">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs text-[var(--muted)]">{doneCount} done · {todos.length} total</span>
          </div>
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            filtered.map((t) => (
              <div
                key={t.id}
                className="flex items-center gap-2.5 px-4 py-2.5 border-b border-[var(--border)] group hover:bg-slate-50 dark:hover:bg-slate-800/50"
              >
                <button onClick={() => toggleTodo(t.id, t.is_completed)} className="shrink-0">
                  <span
                    className={`material-symbols-outlined ${t.is_completed ? "text-[var(--accent)]" : "text-[var(--border)]"}`}
                    style={{ fontSize: "20px", fontVariationSettings: t.is_completed ? "'FILL' 1" : "'FILL' 0" }}
                  >
                    check_circle
                  </span>
                </button>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm ${t.is_completed ? "line-through text-[var(--muted)]" : "text-slate-900 dark:text-white"}`}>
                    {t.text}
                  </p>
