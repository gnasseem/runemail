...
function buildEmailSignalContext(email: EmailInput): { signalLines: string[]; definiteCategory: string | null } {
  ...
  if (email.has_list_unsubscribe) lines.push("Has List-Unsubscribe header: yes (newsletter/bulk signal)");
...
- action-required: a real person is waiting for you to reply, approve, pay, sign, or decide
- important: real person wrote to you, no response required right now
- newsletter: bulk/marketing/promotional, has unsubscribe link
- informational: fully automated system message, no human sender waiting

Use the SIGNALS above to guide your classification.${tagsSection}`,
...
/**
 * summarize: one-sentence email summary.
 */
const summarizeNode: NodeFn = async (state) => {
  ...
  const body = (email.body_text || email.snippet || "").slice(0, 2000);
  const raw = await localInference(
    "Summarize this email in exactly one sentence. Be specific about the key action or information being communicated.",
...
const qaReplyAnalysisNode: NodeFn = async (state) => {
  ...
  const raw = await localInference(
    `Determine if this email needs a reply. If yes, suggest 1-2 specific reply actions.

RULES:
- Replies must be CONVERSATIONAL actions to send TO the sender (accept, decline, ask for info, confirm, thank).
- NEVER suggest replies that are personal notes ("check the doc", "read this") — those are Todos.
- If the email is a notification, receipt, newsletter, or FYI — output [].
- Maximum 2 reply suggestions.

Output JSON array of reply descriptions (no "Reply:" prefix). Example: ["Accept the invitation", "Ask for agenda"]
Return [] if no reply needed.`,
    `From: ${email.sender}\nSubject: ${email.subject}\nContent: ${body}`,
...
const qaTodoAnalysisNode: NodeFn = async (state) => {
  ...
  const raw = await localInference(
    `Extract actionable tasks from this email. Maximum 2.

RULES:
- Tasks must be CONCRETE one-sentence actions (e.g. "Submit registration form by Friday").
- Skip vague tasks ("check email", "read this").
- Do not duplicate existing todos.${existingBlock}

Output JSON array of task descriptions (no "Todo:" prefix). Return [] if nothing actionable.`,
    `Subject: ${email.subject}\nContent: ${body}`,
    { temperature: 0.2, maxTokens: 128 },
  );
...
/**
 * briefing: generate a morning briefing from a list of emails.
 */
const briefingNode: NodeFn = async (state) => {
  ...
  const raw = await localInference(
    `Today's date is ${todayDate}. You are an executive email briefing system. Analyze these emails and return ONLY valid JSON:
{
  "executiveSummary": "<2 sentences: what needs attention today and the main theme>",
  "topPriority": [
    {"subject":"<subject>","senderName":"<name>","sender":"<email>","summary":"<1 sentence why it matters>","urgency":"critical|high|medium","deadline":null,"waitingForReply":true,"tags":[]}
  ],
  "deadlines": [{"task":"<what is due>","date":"<YYYY-MM-DD>","source":"<sender name>"}],
  "waitingForReply": [],
  "stats": {"total": ${total}, "critical": 0, "deadlines": 0, "waitingOnYou": 0, "filtered": 0}
}
Include only emails genuinely needing executive attention in topPriority (max 7). Be specific.`,
    emailsText,
  ...
  const parsed = extractJSON<{
    ...
    deadlines?: DeadlineItem[];
    waitingForReply?: BriefingEmail[];
    stats?: BriefingResult["stats"];
  }>(raw, {});

  // Attach email_id to briefing cards by matching subject against input emails
  const emails = state.emails ?? [];
  const attachIds = (cards: BriefingEmail[]): BriefingEmail[] =>
    cards.map((card) => {
  ...
  return {
    briefing: {
      executiveSummary: parsed.executiveSummary || "Unable to generate briefing summary.",
      topPriority: attachIds(Array.isArray(parsed.topPriority) ? parsed.topPriority : []),
      deadlines: Array.isArray(parsed.deadlines) ? parsed.deadlines : [],
      waitingForReply: attachIds(Array.isArray(parsed.waitingForReply) ? parsed.waitingForReply : []),
      stats: parsed.stats || { total, critical: 0, deadlines: 0, waitingOnYou: 0, filtered: 0 },
    },
  };
};
