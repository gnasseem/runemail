...
async function decryptTokens(encrypted: string): Promise<TokenData> {
  ...
  if (!key) return JSON.parse(encrypted);
  // Plaintext JSON fallback: tokens stored by the callback's direct-insert fallback
  if (encrypted.trimStart().startsWith("{")) {
    try { return JSON.parse(encrypted); } catch { /* not JSON, try Fernet */ }
  }
...
export async function encryptTokens(tokens: TokenData): Promise<string> {
  const key = getEnv("TOKEN_ENCRYPTION_KEY");
  if (!key) return JSON.stringify(tokens);
...
interface ExtractState {
  ...
  attachments: AttachmentInfo[];
  cidParts: { contentId: string; mimeType: string; data?: string; attachmentId?: string }[];
}
...
async function fetchMessageById(
  ...
  try {
    ...
    const state: ExtractState = { bodyText: "", bodyHtml: "", hasAttachments: false, attachments: [], cidParts: [] };
    ...
    for (const cid of state.cidParts) {
      ...
      if (!imgData && cid.attachmentId) {
        try {
          const attRes = await gmailRequest(accessToken, `/messages/${messageId}/attachments/${cid.attachmentId}`);
          if (attRes.ok) {
      ...
      if (imgData && state.bodyHtml) {
        ...
        state.bodyHtml = state.bodyHtml.replace(new RegExp(`cid:${escaped}`, "gi"), dataUri);
      }
    }
    ...
    return {
      ...
      has_list_unsubscribe: !!(headers["list-unsubscribe"] || headers["list-unsubscribe-post"]),
      is_reply: !!(headers["in-reply-to"] || headers["references"]),
      reply_to_email: replyToEmail && replyToEmail !== extractEmail(from) ? replyToEmail : undefined,
...
export async function fetchEmails(
  ...
  // Fetch individual messages in parallel (concurrency = 5) to avoid
  // sequential round-trips that add up to 10-20 s for a 20-message batch.
  const CONCURRENCY = 5;
...
export async function setupGmailWatch(
  ...
  const { tokenData } = await resolveTokens(encryptedTokens, supabase, accountId);
...
export async function sendEmail(
  ...
  if (attachments && attachments.length > 0) {
    ...
    // Simple HTML email
    const headerLines = [...baseHeaders, "Content-Type: text/html; charset=UTF-8"];
...
export async function fetchSentEmails(
  encryptedTokens: string,
  maxResults = 20,
): Promise<{ id: string; subject: string; to: string; date: string; body_html: string; body: string; snippet: string; has_attachments: boolean; attachments: AttachmentInfo[] }[]> {
  const { tokenData } = await resolveTokens(encryptedTokens);
  ...
  const sent: { id: string; subject: string; to: string; date: string; body_html: string; body: string; snippet: string; has_attachments: boolean; attachments: AttachmentInfo[] }[] = [];
  ...
  for (let i = 0; i < messages.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(
      chunk.map(async (stub) => {
        try {
          const msgRes = await gmailRequest(tokenData.token, `/messages/${stub.id}?format=full`);
          if (!msgRes.ok) return null;
          const msg = await msgRes.json();
          const headers: Record<string, string> = {};
          for (const h of msg.payload?.headers ?? []) {
    ...
    for (const r of results) { if (r) sent.push(r); }
  }
  return sent;
}
...
export async function fetchAttachment(
  ...
  const { tokenData } = await resolveTokens(encryptedTokens);
  try {
    const res = await gmailRequest(tokenData.token, `/messages/${messageId}/attachments/${attachmentId}`);
...
export async function fetchDrafts(
  ...
  for (let i = 0; i < draftStubs.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(
      chunk.map(async (d) => {
        try {
          const draftRes = await gmailRequest(tokenData.token, `/drafts/${d.id}`);
